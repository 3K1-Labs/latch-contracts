...
fn install_success() {
    ...
    e.as_contract(&timelock_id, || {
        let params = TimelockAccountParams {
            delay_ledgers: 100,
            cancellable_by: Vec::new(&e),
        };
...
fn enforce_creates_proposal() {
    ...
    // Propose via enforce
    e.as_contract(&timelock_id, || {
        let context = create_call_context(
            &e,
            &target,
            symbol_short!("set"),
            vec![&e, 42u32.into_val(&e)],
        );
        ...
        // Verify events (installed + proposed)
...
fn multiple_proposals_get_unique_ids() {
    ...
    // Propose twice
    e.as_contract(&timelock_id, || {
        let context1 = create_call_context(
            &e,
            &target,
            symbol_short!("set"),
            vec![&e, 1u32.into_val(&e)],
        );
        let context2 = create_call_context(
            &e,
            &target,
            symbol_short!("set"),
            vec![&e, 2u32.into_val(&e)],
        );
        let signers = context_rule.signers.clone();
...
fn cancel_before_unlock_blocks_execution() {
    ...
    e.as_contract(&timelock_id, || {
        let params = TimelockAccountParams {
            delay_ledgers: 100,
            cancellable_by: vec![&e, canceller.clone()],
        };
        crate::install_timelock(&e, &params, &context_rule, &smart_account);
    });
    ...
    // Cancel
    e.as_contract(&timelock_id, || {
        TimelockPolicy::cancel(e.clone(), smart_account.clone(), 0);
    });
...
fn unauthorized_cancel_rejected() {
    ...
    let smart_account = Address::generate(&e);
    let canceller = Address::generate(&e);
    let _unauthorized = Address::generate(&e);
    let context_rule = create_context_rule(&e, &target_id);
